...
# Aangepast voor django production, Bart147, 160228.
...
if settings.PRODUCTIE:
	tmp_folder = settings.TMP_ROOT
	tmp_file = os.path.join(tmp_folder,"tmpheaderdict.pkl")
else:
	tmp_file = "tmpheaderdict.pkl"
...
def get_measurementtext_Tekst(i_Index):
	...
	if 'MEASUREMENTTEXT' in headerdict:
		if i_Index in headerdict['MEASUREMENTTEXT']:
			if len(headerdict['MEASUREMENTTEXT'][i_Index])>0: #
...
def get_zid_flag():
	headerdict=ast.literal_eval(pickle.load(open(tmp_file)))
...
def d_GEF():
    return ast.literal_eval(pickle.load(open(tmp_file)))
